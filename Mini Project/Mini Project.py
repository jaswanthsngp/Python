...
#car(regNo, chasisNo, manufacturer, model, no_of_seats)
#person(name, age, gender, licence, mobile, emergency_mobile)
...
def relieve():
    ...
    mycursor.execute("select tookAt from rent where carNo= %s", (rCar.get(), ))
    t= mycursor.fetchall()
    ### Billing ###
    print(int(time())- t[0][0])
    showinfo('Bill', f'The rent you have to pay is {(int(time())- t[0][0])//60} Rupees')
